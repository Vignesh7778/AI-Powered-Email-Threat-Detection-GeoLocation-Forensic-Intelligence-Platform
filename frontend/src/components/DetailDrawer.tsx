...
export const DetailDrawer: React.FC<DetailDrawerProps> = ({ data, onClose }) => {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md bg-[#12161B] border-l border-[#232A32] shadow-2xl flex flex-col justify-between font-mono text-xs select-text animate-in slide-in-from-right duration-200">
      {/* Header */}
      <div className="p-4 border-b border-[#232A32] flex items-center justify-between bg-[#0A0D10]/80">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded bg-[#191F26] border border-[#232A32]">
            {getIcon()}
          </div>
          <div>
            <div className="font-bold text-[#E7EBEF] text-xs font-sans uppercase tracking-wider flex items-center gap-2">
              <span>{data.type} Telemetry Detail</span>
              <ThreatBadge type="trust" value={data.provenance} size="xs" />
            </div>
            <div className="text-[11px] text-[#8B96A3] truncate max-w-xs font-mono">{data.title}</div>
          </div>
        </div>
      ...
      <div className="p-5 space-y-4 flex-1 overflow-y-auto">
              ...
              <div key={i} className="p-2.5 rounded bg-[#0A0D10] border border-[#232A32] flex items-start justify-between gap-3">
                ...
                {f.isCopyable && (
                  <button
                    ...
                    className="p-1 text-[#8B96A3] hover:text-[#E8A33D] transition-colors flex-shrink-0"
                    title={`Copy ${f.label}`}
                  >
        ...
        {data.evidenceRef && (
          <div className="p-3 rounded bg-[#0A0D10] border border-[#232A32] space-y-1">
            ...
            <p className="text-[11px] text-[#E7EBEF] font-sans">{data.evidenceRef}</p>
          </div>
        )}
      ...
      <div className="p-4 border-t border-[#232A32] bg-[#0A0D10] flex items-center justify-between gap-3">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded bg-[#191F26] border border-[#232A32] text-[#E7EBEF] hover:text-white font-semibold transition-colors"
        >
        ...
        <button
          onClick={onClose}
          className="px-4 py-2 rounded bg-[#E8A33D] hover:bg-[#E8A33D]/90 text-[#0A0D10] font-bold transition-all"
        >
          Done
        </button>
      </div>
