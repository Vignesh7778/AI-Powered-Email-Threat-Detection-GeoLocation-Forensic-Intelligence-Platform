  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 space-y-4 sm:space-y-6 max-w-[1720px] w-full mx-auto bg-[#0A0D10] text-[#E7EBEF]">
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-[#232A32]">
        <div>
          <h1 className="text-base sm:text-lg font-bold text-[#E7EBEF] tracking-tight font-sans">Threat Alert Stream</h1>
      ...
      {/* Filter Bar */}
      <div className="flex flex-wrap items-center justify-between gap-2.5 sm:gap-3 font-mono text-xs bg-[#12161B] border border-[#232A32] p-3 rounded-lg">
      ...
      {/* Main Alerts Presentation (Table on md+, Cards on <md) */}
      <div className="bg-[#12161B] rounded-lg border border-[#232A32] overflow-hidden">
                    ...
                    <td className="py-3 px-4 text-center">
                      {!a.acknowledged ? (
                        <button
                          ...
                          className="px-2.5 py-1 rounded bg-[#0A0D10] border border-[#E8A33D]/40 text-[#E8A33D] hover:bg-[#E8A33D] hover:text-[#0A0D10] text-[10px] font-bold transition-all min-h-[30px]"
                        >
                          Acknowledge
                        </button>
                      ) : (
                        <button
                          onClick={() => onSelectSubmission(a.submission_id)}
                          className="text-[#8B96A3] hover:text-white text-[11px] inline-flex items-center gap-1 min-h-[30px]"
                        >
                          <span>Inspect</span>
        ...
        <div className="md:hidden divide-y divide-[#232A32]">
          {loading ? (
            ...
            filtered.map((a) => (
              <div key={a.alert_id} className="p-3.5 hover:bg-[#191F26] transition-colors space-y-2 font-mono text-xs">
                <div className="flex items-center justify-between gap-2">
                  <ThreatBadge type="risk" value={a.severity} size="xs" />
                  <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${
                    a.acknowledged
                      ? 'bg-[#2DD4BF15] text-[#2DD4BF] border border-[#2DD4BF30]'
                      : 'bg-[#E5484D15] text-[#E5484D] border border-[#E5484D30] animate-pulse'
                  }`}>
                ...
                <div
                  className="text-sm font-semibold text-[#E7EBEF] font-sans cursor-pointer hover:text-[#E8A33D]"
                  onClick={() => onSelectSubmission(a.submission_id)}
                >
                  {a.title}
                </div>

                <div className="text-xs text-[#8B96A3] font-sans line-clamp-2">{a.reason}</div>

                <div className="flex items-center justify-between pt-2 border-t border-[#232A32]">
                  <div className="text-[11px] font-bold text-[#E7EBEF]">
                    Score: {(a.fraud_score * 100).toFixed(0)}/100
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {!a.acknowledged && (
                      <button
                        onClick={() => handleAck(a.alert_id)}
                        className="px-2.5 py-1 rounded bg-[#0A0D10] border border-[#E8A33D]/40 text-[#E8A33D] hover:bg-[#E8A33D] hover:text-[#0A0D10] text-[10px] font-bold transition-all min-h-[32px]"
                      >
                        Acknowledge
                      </button>
                    )}
                    <button
                      onClick={() => onSelectSubmission(a.submission_id)}
                      className="px-2.5 py-1 rounded bg-[#191F26] text-[#E8A33D] hover:bg-[#E8A33D] hover:text-[#0A0D10] text-[10px] font-bold inline-flex items-center gap-1 transition-all min-h-[32px]"
                    >
                      <span>Inspect</span>
                      <ArrowRight className="w-3 h-3" />
                    </button>
