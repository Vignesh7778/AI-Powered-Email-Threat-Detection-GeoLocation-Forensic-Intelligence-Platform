  ...
  return (
    <div className="p-3 sm:p-5 lg:p-6 space-y-4 sm:space-y-6 max-w-[1720px] w-full mx-auto bg-[#0A0D10] text-[#E7EBEF]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-[#232A32]">
        <div>
          <h1 className="text-base sm:text-lg font-bold text-[#E7EBEF] tracking-tight font-sans">
            Incident & Forensic Case Desk
          </h1>
          <p className="text-[11px] sm:text-xs font-mono text-[#8B96A3] mt-0.5">
            Structured SOC workflow management, lifecycle state tracking, and evidentiary preservation
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-[#E8A33D] text-[#0A0D10] text-xs font-bold font-mono hover:bg-[#E8A33D]/90 shadow-sm transition-all min-h-[36px]"
          >
          ...
          <button
            ...
            disabled={refreshing}
            className="p-2 rounded bg-[#12161B] border border-[#232A32] text-[#8B96A3] hover:text-[#E8A33D] transition-colors min-h-[36px] min-w-[36px] flex items-center justify-center"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-[#12161B] border border-[#232A32] p-3 rounded-lg font-mono text-xs">
        <div className="flex flex-wrap items-center gap-2.5 flex-1 min-w-[200px]">
          ...
          <div className="relative flex-1 min-w-[180px] max-w-sm">
            ...
            <input
              ...
              placeholder="Search case, title, analyst..."
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[#8B96A3] hover:text-white"
              >
        ...
        <div className="flex items-center gap-3">
          <div className="text-[11px] text-[#8B96A3] hidden sm:inline">
            <span className="text-[#E8A33D] font-bold">{filteredCases.length}</span> Active Cases
          </div>

          {/* View Mode Toggle */}
          <div className="flex items-center bg-[#0A0D10] border border-[#232A32] rounded p-0.5">
            <button
              onClick={() => setViewMode('kanban')}
              className={`p-1.5 rounded transition-colors ${viewMode === 'kanban' ? 'bg-[#191F26] text-[#E8A33D]' : 'text-[#8B96A3] hover:text-white'}`}
            ...
            <button
              ...
              className={`p-1.5 rounded transition-colors ${viewMode === 'table' ? 'bg-[#191F26] text-[#E8A33D]' : 'text-[#8B96A3] hover:text-white'}`}
              title="Table View"
            >
      ...
      {loading ? (
        ...
        <div className="w-full h-80 bg-[#12161B] border border-[#232A32] rounded-lg flex flex-col items-center justify-center p-6 sm:p-8 text-center space-y-3 font-mono">
          ...
          <button
            onClick={() => setIsCreateModalOpen(true)}
            className="px-4 py-2 rounded bg-[#E8A33D] text-[#0A0D10] text-xs font-bold font-mono min-h-[38px]"
          >
            + Create First Case
          </button>
        </div>
      ) : viewMode === 'kanban' ? (
        /* Responsive Kanban: Swipeable Columns on Mobile, 5-col Grid on Desktop */
        <div className="flex md:grid md:grid-cols-5 gap-3.5 items-start overflow-x-auto pb-3 md:pb-0 scrollbar-none snap-x snap-mandatory">
          {STAGES.map((stage) => {
            const stageCases = filteredCases.filter(c => (c.status || 'OPEN').toUpperCase() === stage);
            return (
              <div
                key={stage}
                className="w-72 md:w-auto flex-shrink-0 snap-center bg-[#12161B] rounded-lg border border-[#232A32] p-3 flex flex-col justify-between font-mono text-xs min-h-[480px]"
              >
                {/* Column Header */}
                ...
                {/* Cards Container */}
                <div className="space-y-2.5 flex-1 overflow-y-auto pr-0.5 max-h-[560px]">
                  {stageCases.length > 0 ? (
                    stageCases.map((c) => (
                      <div
                        key={c.case_id}
                      ...
                      >
                        ...
                        <div className="pt-1.5 border-t border-[#232A32]/60 flex items-center justify-between gap-1 text-[10px]">
                          ...
                          <select
                            ...
                            className="bg-[#12161B] border border-[#232A32] text-[#8B96A3] hover:text-[#E7EBEF] rounded px-1.5 py-1 text-[10px] focus:outline-hidden"
                          >
                            {STAGES.map(s => (
                          ...
                          {c.submission_ids?.[0] && (
                            <button
                              ...
                              className="text-[#2DD4BF] hover:underline flex items-center gap-0.5 font-bold p-1"
                            >
                              <span>Inspect</span>
              ...
              <tbody className="divide-y divide-[#232A32]">
                {filteredCases.map((c) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-[#E7EBEF] font-sans font-semibold truncate max-w-sm">
                      {c.title}
                    </td>
                    <td className="py-3 px-4">
                      <select
                        ...
                        onClick={(e) => e.stopPropagation()}
                        onChange={(e) => handleStatusChange(c.case_id, e.target.value)}
                        className="bg-[#0A0D10] border border-[#232A32] text-[#E8A33D] rounded px-2 py-0.5 text-[10px] font-bold focus:outline-hidden"
                      >
                        {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </td>
                    <td className="py-3 px-4 text-[#8B96A3] font-sans">{c.assigned_analyst || 'analyst@org.gov'}</td>
