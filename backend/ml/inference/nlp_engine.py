...
from backend.app.schemas.schemas import DetectedPattern

class NLPEngine:
    """
    NLP Social Engineering & Content Analysis Engine.
    Detects urgency cues, executive impersonation, fake invoice fraud,
    payment diversion, and credential harvesting patterns.
    Extracts exact character offset spans [start, end] in the original body text.
    """
    ...
    def analyze(self, subject: str, body_text: str) -> Dict[str, Any]:
        ...
        for p_type, rule_list in self.PATTERNS.items():
            for rule in rule_list:
                ...
                # Search body text for character offset spans
        ...
        # Non-linear aggregate urgency
        if urgency_signals:
            combined = 0.0
            for s in urgency_signals:
                combined = combined + s * (1.0 - combined) * 0.7
            urgency_score = round(min(max(combined, 0.05), 0.99), 2)
        else:
            urgency_score = 0.05

        # Non-linear aggregate impersonation
        if impersonation_signals:
            combined = 0.0
            for s in impersonation_signals:
                combined = combined + s * (1.0 - combined) * 0.75
            impersonation_score = round(min(max(combined, 0.05), 0.99), 2)
        else:
